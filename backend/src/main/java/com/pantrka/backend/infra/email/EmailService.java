package com.pantrka.backend.infra.email;
...
public class EmailService {
    ...
    public void sendPasswordResetCode(String email, String code) {
        try {
            ...
            helper.setTo(email);
            helper.setSubject("[PanTrKa] 비밀번호 재설정 인증번호");
    ...
    public void sendPaymentConfirmEmail(OrderEmailContext ctx) {
        send(ctx.email(),
                "[PanTrKa] 주문이 결제되었습니다 (" + ctx.orderNumber() + ")",
    ...
    public void sendShippedEmail(OrderEmailContext ctx) {
        send(ctx.email(),
                "[PanTrKa] 주문하신 상품이 발송되었습니다 (" + ctx.orderNumber() + ")",
    ...
    public void sendAdminCancelEmail(OrderEmailContext ctx) {
        send(ctx.email(),
                "[PanTrKa] 주문이 취소되었습니다 (" + ctx.orderNumber() + ")",
                        ...
                        <table width="480" cellpadding="0" cellspacing="0" style="background-color:#1e1e1e; border-radius:8px; overflow:hidden;">
                          <tr>
                            <td style="padding:40px 40px 20px 40px; text-align:center;">
                              <h1 style="color:#ffffff; font-size:20px; letter-spacing:0.2em; font-weight:300; margin:0;">PanTrKa</h1>
                            </td>
                          </tr>
                          <tr>
