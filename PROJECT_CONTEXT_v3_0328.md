...
16. domain/auth/entity/Member.java, MemberAddress.java, Role.java, Provider.java
...
19. domain/auth/ 전체 레이어 구현 완료 (Repository, DTO, Service, Controller)
20. Swagger UI 정상 작동 확인
21. 회원가입/로그인 API 실제 테스트 완료 (201/200 응답 확인)

### 미완료 항목 ❌ (다음 작업 순서)
1. Product 도메인 API (Repository → DTO → Service → Controller)
   - GET /api/products (목록, 필터/정렬/페이지)
   - GET /api/products/{id} (상세)
   - GET /api/categories (카테고리 트리)
2. Order 도메인 API (장바구니, 주문 생성, 주문 취소)
3. Payment 도메인 API (토스페이먼츠 연동)
4. Admin 도메인 API (상품 등록/수정/삭제, 주문 상태 변경)
5. User 도메인 API (내 정보 조회/수정, 배송지 관리)
6. 테스트 → 배포
